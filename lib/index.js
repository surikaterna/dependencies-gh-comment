...
// src/commentOnPullRequest.ts
var core = __toESM(require_core());
var github = __toESM(require_github());
var commentOnPullRequest = async (markdown) => {
...
// src/assertDependencies.ts
var assertDependencies = (input) => {
  if (!Array.isArray(input)) {
    return false;
  }
  return input.every(assertDependency);
};
var assertDependency = (value) => assertNonNullObject(value) && assertDependencyProperties(value);
var assertNonNullObject = (value) => typeof value === "object" && value !== null;
var assertDependencyProperties = (obj) => {
...
// src/createMarkdown.ts
var createMarkdown = (dependencies) => ["|Name|Version|License|", "|---|---|---|"].concat(dependencies.map((d) => `|${d.name}|${d.version}|${d.license}|`)).join("\n");

// src/index.ts
async function run() {
  try {
    const input = core2.getInput("dependencies");
